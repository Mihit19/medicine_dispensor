...
class EEAppBar extends StatelessWidget implements PreferredSizeWidget {
  const EEAppBar(
      {super.key,
      this.title,
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      padding: const EdgeInsets.symmetric(horizontal: EESizes.md),
      child: AppBar(
          automaticallyImplyLeading: false,
          leading: showBackArrow
              ? IconButton(
                  onPressed: () => Get.back(),
                  icon: Icon(Iconsax.arrow_left, color: dark? EEColors.white:EEColors.dark,))
